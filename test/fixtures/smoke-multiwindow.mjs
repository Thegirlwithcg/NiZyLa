...
try {
  ...
  assert.ok(promptMsg?.includes('replacing it with the conversion result'), 'Prompt should ask about replacing with conversion result');
  await ui.screenshot('k_convert_prompt_replace');

  // Cancel -> abort, keep edits
  await ui.evaluate(`Array.from(document.querySelectorAll('.modal-actions button')).find(b => b.textContent.trim() === 'Cancel')?.click()`);
  await delay(400);
  ...
  await ui.evaluate(`Array.from(document.querySelectorAll('.modal-actions button')).find(b => b.textContent.trim() === "Don't Save")?.click()`);
  await delay(800);
